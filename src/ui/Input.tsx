import { type InputHTMLAttributes, forwardRef, useId } from 'react'
import { cn } from '@/lib/cn'

export interface InputProps extends InputHTMLAttributes<HTMLInputElement> {
  label?: string
  error?: string
  hint?: string
}

export const Input = forwardRef<HTMLInputElement, InputProps>(
  ({ className, label, error, hint, id: externalId, ...props }, ref) => {
    const generatedId = useId()
    const id = externalId ?? generatedId
    const errorId = `${id}-error`
    const hintId = `${id}-hint`

    return (
      <div className="flex flex-col gap-1">
        {label && (
          <label htmlFor={id} className="text-label uppercase text-[var(--text-tertiary)]">
            {label}
          </label>
        )}
        <input
          ref={ref}
          id={id}
          aria-describedby={cn(error && errorId, hint && hintId) || undefined}
          aria-invalid={!!error}
          className={cn(
            'h-8 w-full rounded-sm border bg-[var(--bg-input)] px-3 text-body text-[var(--text-primary)] placeholder:text-[var(--text-muted)]',
            'border-[var(--border-default)] transition-colors',
            'focus:border-[var(--action-primary)] focus:outline-none focus:ring-1 focus:ring-[var(--action-primary)]',
            error && 'border-[var(--state-error)] focus:border-[var(--state-error)] focus:ring-[var(--state-error)]',
            props.disabled && 'cursor-not-allowed opacity-50',
            className,
          )}
          {...props}
        />
        {hint && !error && (
          <p id={hintId} className="text-caption text-[var(--text-muted)]">
            {hint}
          </p>
        )}
        {error && (
          <p id={errorId} role="alert" className="text-caption text-[var(--state-error-text)]">
            {error}
          </p>
        )}
      </div>
    )
  },
)
Input.displayName = 'Input'
